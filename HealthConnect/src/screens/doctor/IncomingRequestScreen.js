...
const IncomingRequestScreen = ({ route, navigation }) => {
  ...
  const handleAccept = async () => {
    try {
      if (request.id) {
        const apptRef = doc(db, 'appointments', String(request.id));
        await updateDoc(apptRef, { status: 'confirmed' });
      }
      navigation.replace('PatientChat', { appointment: { ...request, status: 'confirmed' } });
    } catch (error) {
      console.error('Error accepting request in Firestore:', error);
    }
  };
  ...
  const handleDecline = async () => {
    try {
      ...
      console.error('Error declining request in Firestore:', error);
    }
  };
