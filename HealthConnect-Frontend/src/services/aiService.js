...
export const processAITriage = async (queryText, chatHistory = []) => {
  // 1. Try real AI backend first (Groq Llama 3.3 70B Engine)
  try {
    const payload = {
      symptoms: queryText,
      history: chatHistory.map(m => ({
        role: m.role || (m.isMe ? 'user' : 'assistant'),
    ...
    if (response.data && response.data.reply) {
      return {
        ...
        severity: response.data.severity || 'normal',
        suggestedPrompts: response.data.suggestedPrompts || [
          "Kon doctor ke dekhabo?",
          "Aro kichu tips din",
          "Appointment book korbo"
        ]
      };
    }
  } catch (apiError) {
    console.warn('Backend AI API error or offline, falling back to smart local triage:', apiError.message);
  }

  // 2. Intelligent local fallback if backend is down
